...
if ($ENV{'OSTYPE'} eq 'linux') {
   ...
   $CPP = "cpp-4.0 -P -xassembler-with-cpp";
};
...
if ( $load eq 'dyn_case' or $load eq 'dyn_file' ) {
   (require "$dir/dynamic_test.pl" && require "$dir/sub_dyn.pl") || die $msg;
} elsif ( $load eq 'sta_case' or $load eq 'sta_file') {
   (require "$dir/static_test.pl") || die $msg;
} elsif ( $load eq 'se_testdriver' && $ENV{'USER'} eq 'budde' ) {
   print "Hallo Reinhard!\n"; exit (0);
} elsif ( $load eq 'regression' ) {
   (require "$dir/regression.pl") || die $msg;
...
if ($load eq 'sta_case' && $case eq '') {
   $case = 'O1';
} elsif ($load eq 'dyn_case' && $case eq '') {
...
if (($load eq 'sta_file' or $load eq 'dyn_file') && $case ne '') {
   log_exit ('sta_file or dyn_file may not be used with a test case');
};
