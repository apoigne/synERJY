...
####### process the sequence of trace-definitions
   ...
   while ($process_trace && ($line = <IN>)) {
      if ( $line =~ /^\s*\/\/\s*$/) {
         ...
         $next_trace_name = $1;
         #&check_trace ();    ## check:    output trace, then more traces
         &output_trace ();    ## check:    output trace, then more traces
         $trace_name = $next_trace_name;
         @instants = ();
      } elsif ( !($line =~ /^\s*\/\//) ) {
         #&check_trace ();
         &output_trace ();
         $process_trace = 0; ## stop:      output trace, no more traces
      } else {
...
####### ifdef found, now generate the binary
   ...
   $cmd = "$SE_HOME/testsuite/bin/v_se_run_cmp";
   $cmd = "$cmd $SEF $SE_HOME None true $SE_COMP verilog $SETR true |";
   #$cmd = "$cmd $SEF $SE_HOME None true $SE_COMP |";
